import {streetOffsets,streetCopies} from "./custom/street-motion.mjs";
import {BackendScene} from "./custom/CareerSections.js";
import {
  j as s,
  S as ps,
  C as I,
...
function Js({ items: e, variant: a = "solid" }) {
  ...
  j.useEffect(() => {
    ...
    const B = (C) => {
      ...
      ((d = C),
        (u = u + (T * E) / 1e3),
        (y.style.transform = `translateX(${(-streetOffsets(u,S).mid).toFixed(2)}px)`),
        (g.style.transform = `translateX(${(-streetOffsets(u,S).far).toFixed(2)}px)`),
        (m.style.transform = `translateX(${(-streetOffsets(u,S).near).toFixed(2)}px)`));
  ...
  return s.jsxs(V, {
    ...
    children: [
      ...
      s.jsxs("div", {
        ...
        onClick: F,
        role: "img",
        "aria-label": `Um rapaz loiro caminha pela cidade, passando por ${N} equipes: ${e.map((g) => g.name).join(", ")}. Clique para ele cumprimentar.`,
        children: [
          s.jsxs("svg", {
            viewBox: `0 0 ${w} ${os}`,
            ...
            children: [
              ...
              s.jsx("g", {
                className: "mid",
                ref: i,
                children: Array.from({length:streetCopies(w,S)},(_,index)=>index).map((g) =>
                  s.jsxs(
                    "g",
                    {
...
function ce({ shots: e, lead: a }) {
  const t = Q("(prefers-reduced-motion: reduce)"),
    ...
    i = j.useMemo(() => {
      ...
      return (
        d.splice(f, 0, { src: a, alt: "Projetos" }),
        d.slice(0, x * hs)
      );
    }, [e, x, f, a]),
    p = j.useRef(null),
...
function be() {
  ...
  const x = fs.map((n) => ({
      title: n.title,
      years: `${n.yr}${n.unit.replace("+ yrs", "+ ANOS").toUpperCase()}`
        .replace(/\s+/g, " ")
        .trim(),
      desc: n.desc,
